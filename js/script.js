...
let pythonhtml = setInterval(() => {
  pythonStartValue++;

  pythonValue.textContent = `${pythonStartValue}%`;
  pythonProgress.style.background = `conic-gradient(#fca61f ${pythonStartValue * 3.6
    }deg, #ededed 0deg)`;

  if (pythonStartValue == pythonEndValue) {
    clearInterval(pythonhtml);
  }
}, pythonspeed);
...
let sqlhtml = setInterval(() => {
  sqlStartValue++;

  sqlValue.textContent = `${sqlStartValue}%`;
  sqlProgress.style.background = `conic-gradient(#7d2ae8 ${sqlStartValue * 3.6
    }deg, #ededed 0deg)`;

  if (sqlStartValue == sqlEndValue) {
    clearInterval(sqlhtml);
  }
}, sqlspeed);

let gProgress = document.querySelector(".g"),
  gValue = document.querySelector(".g-progress");

let gStartValue = 0,
  gEndValue = 90,
  gspeed = 30;

let ghtml = setInterval(() => {
  gStartValue++;

  gValue.textContent = `${gStartValue}%`;
  gProgress.style.background = `conic-gradient(#20c997 ${gStartValue * 3.6
    }deg, #ededed 0deg)`;
...
let mProgress = document.querySelector(".m"),
  mValue = document.querySelector(".m-progress");

let mStartValue = 0,
  mEndValue = 70,
  mspeed = 30;

let mhtml = setInterval(() => {
  mStartValue++;

  mValue.textContent = `${mStartValue}%`;
  mProgress.style.background = `conic-gradient(#3f396d ${mStartValue * 3.6
    }deg, #ededed 0deg)`;
